		...
		return PaginationView = (function() {
			...
			PaginationView.prototype.buildLinks = function() {
				...
				if (this.settings.prev) {
					links.push(this.buildLi(current_page - 1,
							this.settings.prev));
				}
				_ref = this.pages();
				...
				if (this.settings.last) {
					links.push(this.buildLi(this.settings.total_pages,
							this.settings.last));
				}
				return links;
			};
			...
			PaginationView.prototype.buildLink = function(page, text) {
				...
				return "<a href='javascript:' data-page='" + page + "'"
						+ data_attr + ">" + text + "</a>";
			};
			PaginationView.prototype.pages = function() {
				...
				if (total_pages > 10) {
					...
					if (current_page === total_pages) {
						for (page = _i = _ref = total_pages - max; _ref <= total_pages ? _i <= total_pages
								: _i >= total_pages; page = _ref <= total_pages ? ++_i
								: --_i) {
							pages.push(page);
						}
					}
					if (total_pages - current_page < max - 1) {
						for (page = _j = _ref1 = total_pages - max; _ref1 <= total_pages ? _j <= total_pages
								: _j >= total_pages; page = _ref1 <= total_pages ? ++_j
								: --_j) {
							pages.push(page);
						}
					} else if (current_page > max - 1) {
						buf = max / 2;
						for (page = _k = _ref2 = current_page - buf,
								_ref3 = current_page + buf; _ref2 <= _ref3 ? _k <= _ref3
								: _k >= _ref3; page = _ref2 <= _ref3 ? ++_k
								: --_k) {
							pages.push(page);
						}
					} else if (current_page <= max - 1) {
						for (page = _l = 2; 2 <= max ? _l <= max : _l >= max; page = 2 <= max ? ++_l
								: --_l) {
							pages.push(page);
						}
					}
					...
					for (page = _m = 1; 1 <= total_pages ? _m <= total_pages
							: _m >= total_pages; page = 1 <= total_pages ? ++_m
							: --_m) {
						pages.push(page);
					}
				}
			...
			PaginationView.prototype.render = function() {
				...
				this.el.html("");
				if (this.settings.total_pages === 1
						&& this.settings.ignore_single_page) {
					return;
				}
				...
				$("[data-page='..']", this.el).closest("li").addClass(
						"disabled");
				$("[data-page='0']", this.el).closest("li")
						.addClass("disabled");
				$("[data-page='" + (this.settings.total_pages + 1) + "']",
						this.el).closest("li").addClass("disabled");
				if (this.settings.current_page === 1 && this.settings.first) {
					$("li:first", this.el).removeClass("active").addClass(
							"disabled");
				}
				if (this.settings.current_page === this.settings.total_pages
						&& this.settings.last) {
					return $("li:last", this.el).removeClass("active")
							.addClass("disabled");
				}
			};
			PaginationView.prototype.isValidPage = function(page) {
				return page > 0 && page !== this.settings.current_page
						&& page <= this.settings.total_pages;
			};
